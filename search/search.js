document.addEventListener('DOMContentLoaded', () => {
    ...
    function findWordMatches(text, queryWords) {
        ...
        const isWordChar = /[a-zA-Z0-9_\u0400-\u04FF\u0590-\u05FF-]/;
        ...
        queryWords.forEach((word, wordIdx) => {
            ...
            while ((match = regex.exec(text)) !== null) {
                ...
                const isPrevWordChar = isWordChar.test(prevChar);
                const isNextWordChar = isWordChar.test(nextChar);
                
                // Enforce strict boundaries for short words
                if (word.length <= 3) {
                ...
                matches.push({ start, end, word, wordIdx });
            }
        });
    ...
    function filterOverlappingMatches(matches) {
        ...
        for (const match of matches) {
            if (match.start >= lastEnd) {
                ...
                // Overlap! Keep the longer match if possible
    ...
    function groupMatches(matches, maxDistance = 30) {
        ...
        for (let i = 1; i < matches.length; i++) {
            ...
            if (currentMatch.start <= lastMatch.end + maxDistance) {
    ...
    function getMatchLevelClass(uniqueCount, totalWords) {
        if (totalWords <= 1) return 'match-default';
        
        const missing = totalWords - uniqueCount;
        if (missing === 0) return 'match-full';
        if (missing === 1) return 'match-minus-1';
        if (missing === 2) return 'match-minus-2';
        if (missing === 3) return 'match-minus-3';
        return 'match-low';
    }

    function highlightTextWithGroups(text, query) {
        const queryWords = getQueryWords(query);
        if (queryWords.length === 0) return text;
        
        let matches = findWordMatches(text, queryWords);
        matches = filterOverlappingMatches(matches);
        if (matches.length === 0) return text;
        
        const groups = groupMatches(matches);
        const matchToClass = new Map();
        for (const group of groups) {
            const uniqueCount = new Set(group.map(m => m.wordIdx)).size;
            const className = getMatchLevelClass(uniqueCount, queryWords.length);
            for (const match of group) {
                matchToClass.set(match, className);
            }
        }
        ...
        let lastIdx = 0;
        for (const match of matches) {
            const className = matchToClass.get(match);
            result += text.substring(lastIdx, match.start);
            result += `<mark class="${className}">${text.substring(match.start, match.end)}</mark>`;
            lastIdx = match.end;
        }
    ...
    function searchDocument(query, doc) {
        ...
        // Apply coordination factor to prioritize matching more unique terms
        const coord = matchedTerms.length / queryTerms.length;
        score = score * coord;
    ...
    function checkPhraseInText(text, query) {
        ...
        // 1. Проверяем точное совпадение всей поисковой фразы (включая стоп-слова)
        if (lowerText.includes(cleanQuery)) {
            return { bonus: 1200, type: 'exact' };
        }
        
        const queryWords = getQueryWords(query);
        if (queryWords.length < 1) return { bonus: 0, type: 'none' };
        
        // 2. Для одиночных слов проверяем обычное вхождение
        ...
        // 3. Для фраз из 2+ слов: группируем слова и оцениваем лучшую группу
        let matches = findWordMatches(text, queryWords);
        matches = filterOverlappingMatches(matches);
        ...
        const groups = groupMatches(matches, 30);
        ...
        const missing = totalWords - maxUniqueCount;
        
        if (missing === 0) {
            return { bonus: 1000, type: 'exact' }; // Совпали все слова рядом
        } else if (missing === 1) {
            return { bonus: 600, type: 'proximity' }; // Без одного слова
        } else if (missing === 2) {
            return { bonus: 300, type: 'ordered' }; // Без двух слов
        } else if (maxUniqueCount >= 2) {
            return { bonus: 100, type: 'scattered' }; // Найдено хотя бы 2 слова рядом
        }
    ...
    async function enhanceResultsWithPhraseSearch(results, query) {
        // Для топ-100 результатов проверяем точные фразы
    ...
    function createBookGroupItem(bookGroup) {
        ...
        bookItem.innerHTML = `
            ...
            <div class="chapter-list">
                ${bookGroup.chapters.map((ch, idx) => `
                    <div class="chapter-item ${idx >= maxVisibleChapters ? 'hidden-chapter' : ''}" 
                         style="${idx >= maxVisibleChapters ? 'display: none;' : ''}"
    ...
    function extractSnippets(fullMarkdown, result, query) {
        ...
        // Находим все вхождения терминов в тексте
        let matches = findWordMatches(plainText, queryWords);
        matches = filterOverlappingMatches(matches);
        ...
        // Группируем совпадения
        const groups = groupMatches(matches, 30);
        
        // Сортируем группы по качеству совпадения: сначала те, где больше уникальных слов запроса, затем по общему числу слов
        groups.sort((a, b) => {
            const uniqueA = new Set(a.map(m => m.wordIdx)).size;
            const uniqueB = new Set(b.map(m => m.wordIdx)).size;
            if (uniqueB !== uniqueA) return uniqueB - uniqueA;
            return b.length - a.length;
        });

        // Берем до 3 лучших групп для сниппетов
        const topGroups = groups.slice(0, 3);
        
        const snippets = topGroups.map(group => {
            // Границы группы
            const groupStart = Math.min(...group.map(m => m.start));
            const groupEnd = Math.max(...group.map(m => m.end));
            
            const position = { start: groupStart, end: groupEnd };
            const snippetText = extractContext(plainText, position, 100);
            
            return highlightTextWithGroups(snippetText, query);
        });

        return snippets.map(s => `<div class="snippet">${s}</div>`).join('');
    }
    ...
    resultsContainer.addEventListener('click', async (e) => {
        // Click on more-chapters button
        const moreBtn = e.target.closest('.more-chapters');
        if (moreBtn) {
            const chapterList = moreBtn.closest('.chapter-list');
            if (chapterList) {
                const isExpanded = chapterList.classList.toggle('expanded');
                if (isExpanded) {
                    moreBtn.textContent = 'Collapse';
                    moreBtn.classList.add('collapsed-state');
                    // Show all hidden chapters
                    chapterList.querySelectorAll('.hidden-chapter').forEach(ch => {
                        ch.style.display = 'flex';
                    });
                } else {
                    const count = moreBtn.dataset.count;
                    moreBtn.textContent = `+${count} more chapters in this book...`;
                    moreBtn.classList.remove('collapsed-state');
                    // Hide extra chapters
                    chapterList.querySelectorAll('.hidden-chapter').forEach(ch => {
        ...
        const resultItem = e.target.closest('.result-item');
        if (resultItem && !e.target.classList.contains('button') && !e.target.closest('.chapter-list')) {
            openModal(resultItem.dataset, currentQuery);
            return;
        }
    });
    ...
    function displayModalContent(markdown, query) {
        ...
        modalBody.innerHTML = marked.parse(markdown, { baseUrl: baseUrlForAssets });
        ...
        if (query) {
            highlightDomElement(modalBody, query);
            ...
            if (highlightedCount > 0) {
                ...
                infoBar.innerHTML = `
                    ...
                    They are automatically expanded and highlighted.
                `;
    ...
    function highlightDomElement(element, query) {
        ...
        textNodes.forEach(textNode => {
            ...
            const highlightedHtml = highlightTextWithGroups(text, query);
    ...
    function highlightAndOpenDetails(query) {
        ...
        detailsElements.forEach(detailsEl => {
            const marks = detailsEl.querySelectorAll('mark');
            if (marks.length > 0) {
                ...
                marks.forEach(m => matchedWords.add(m.textContent.trim().toLowerCase()));
                const searchTerm = Array.from(matchedWords).slice(0, 3).join(', ');
