document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const rawChapter = params.get('chapter') || 'chapter-01';
...
async function initReader(bookPath, chapterId, edition) {
    try {
        ...
        const metadataUrl = `${isGitHubPages ? rawBase : (typeof BASE_URL !== 'undefined' ? BASE_URL : './')}${bookPath}/metadata.json?t=${Date.now()}`;
        ...
        try {
            ...
            localStorage.setItem('starley_last_reader_url', window.location.href);
        } catch (e) {
