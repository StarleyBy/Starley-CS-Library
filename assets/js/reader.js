...
async function loadChapter(bookPath, chapterId, edition) {
    ...
    try {
        ...
        const relativeImagesDir = `${bookPath}/chapters/${imagesFolder}/images/`;
        let imgBaseUrl = typeof window.getImageUrl === 'function'
            ? window.getImageUrl(relativeImagesDir)
            : `${BASE_URL}${relativeImagesDir}`;
        ...
        setTimeout(() => {
            area.querySelectorAll('img').forEach(img => {
                ...
                if (rawSrc) {
                    const r2Prefix = window.R2_MEDIA_BASE_URL ? window.R2_MEDIA_BASE_URL.replace(/\/$/, '') : '';
                    if (!r2Prefix || !rawSrc.startsWith(r2Prefix)) {
                        const fileName = rawSrc.split('?')[0].split('#')[0].split('/').pop();
                        const relativePath = `${bookPath}/chapters/${imagesFolder}/images/${fileName}`;
                        if (typeof window.getImageUrl === 'function') {
                            img.src = window.getImageUrl(relativePath);
                        }
                    }
                }
                img.onerror = function() {
                    this.onerror = null;
                    this.src = 'assets/img/book-placeholder.png';
                    this.classList.add('cover-fallback');
                };
            ...
            // Initialize / Refresh Dynamic HUD Table of Contents
            if (typeof window.TocHud !== 'undefined') {
                if (window.currentTocHud) {
                    window.currentTocHud.init();
                } else {
                    window.currentTocHud = new window.TocHud({ container: '#content-area', headerOffset: 80 });
                }
            }

            // Trigger Deep-Link Jump & Pulsing Neon Highlight for Search terms
            _handleDeepLinkJump();

            // Initialize / Refresh Smart Highlighter & Notes Engine
            if (typeof window.HighlighterEngine !== 'undefined') {
                if (window.currentHighlighterToolbar) {
                    window.currentHighlighterToolbar.destroy();
                }
                window.currentHighlighterEngine = new window.HighlighterEngine('#content-area', bookPath, chapterId);
                if (typeof window.HighlightToolbar !== 'undefined') {
                    window.currentHighlighterToolbar = new window.HighlightToolbar(window.currentHighlighterEngine);
                }
                // Save last active edition for My Highlights page context
                try {
                    localStorage.setItem('reader_last_edition', edition || 'original');
                    const hlLink = document.querySelector('a[href^="highlights.html"]');
                    if (hlLink) {
                        hlLink.href = `highlights.html?edition=${edition || 'original'}`;
                    }
                } catch (e) {}
            }

            _handleDeepLinkJump();
            window.dispatchEvent(new CustomEvent('reader:chapterChanged'));
        }, 0);
    } catch (e) {
...
// ==========================================================================
//  TYPOGRAPHY PRESETS MANAGER — Anti-CLS & LocalStorage persistence
// ==========================================================================
...
const TypographyManager = {
    ...
    init() {
        const saved = (window.ReaderSettingsManager && window.ReaderSettingsManager.getPreset()) ||
                      ReaderSettings.get('reader_typography_preset', 'modern');
        this.applyPreset(saved, false);
    },

    setPreset(presetId) {
        if (!presetId) return;
        if (presetId === this.currentPreset) {
            this.applyPreset(presetId, false);
            return;
        }
        this.applyPresetWithScrollCompensation(presetId);
    },

    applyPreset(presetId, compensateScroll = true) {
        ...
        const targets = [
            ...
            document.getElementById('radial-menu-container')
        ];
        targets.forEach(el => {
            if (el) el.setAttribute('data-typography-preset', presetId);
        });

        if (window.ReaderSettingsManager) {
            window.ReaderSettingsManager.savePreset(presetId);
        } else {
            ReaderSettings.set('reader_typography_preset', presetId);
        }

        if (window.RadialMenu) {
            window.RadialMenu.syncActivePresetUI();
        }
    },

    applyPresetWithScrollCompensation(presetId) {
        const contentArea = document.getElementById('content-area') || document.body;
        const blocks = Array.from(contentArea.querySelectorAll('p, h1, h2, h3, h4, blockquote, li, td'));
        ...
        if (!anchorEl && blocks.length > 0) {
            anchorEl = blocks.reduce((prev, curr) => {
                return Math.abs(curr.getBoundingClientRect().top) < Math.abs(prev.getBoundingClientRect().top) ? curr : prev;
            });
            if (anchorEl) oldTop = anchorEl.getBoundingClientRect().top;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    _initRadialMenu();
    _initFavorites();
    _initDisciplineAccentStripe();

    const tocToggleBtn = document.getElementById('toc-toggle');
    if (tocToggleBtn) {
        tocToggleBtn.title = "Toggle Chapter Navigation Sidebar";
        tocToggleBtn.addEventListener('click', function() {
            document.body.classList.toggle('sidebar-open');
        });
    }

    console.log('✅ Smart navigation & Typography Presets active');
});

function _initDisciplineAccentStripe() {
    try {
        const params = new URLSearchParams(window.location.search);
        const themeColor = params.get('theme') || sessionStorage.getItem('activeThemeColor');
        if (themeColor) {
            const stripe = document.getElementById('discipline-accent-stripe');
            if (stripe) {
                stripe.style.backgroundColor = decodeURIComponent(themeColor);
...
function _initRadialMenu() {
    ...
    trigger.addEventListener('click', (e) => {
        ...
        // If in typography submenu state, return to main radial state
        if (window.RadialMenu && window.RadialMenu.currentState === 'TYPOGRAPHY') {
            window.RadialMenu.exitSubmenuState();
            return;
        }
    ...
    document.querySelectorAll('.radial-item').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            else if (action === 'typography-menu' || action === 'font-pick') { 
                if (window.RadialMenu) {
                    window.RadialMenu.enterTypographyState();
                } else if (fontPicker) {
            ...
            else if (action === 'search-open')     { window.location.href = 'search/index.html'; _closeMenu(); }
    ...
    if (editionPicker) {
        ...
        const bookPath = params.get('book');
        if (bookPath) {
            const rawBase = (typeof RAW_CONTENT_BASE_URL !== 'undefined') ? RAW_CONTENT_BASE_URL : (typeof BASE_URL !== 'undefined' ? BASE_URL : './');
            const metadataUrl = `${window.location.hostname.includes('github.io') ? rawBase : (typeof BASE_URL !== 'undefined' ? BASE_URL : './')}${bookPath}/metadata.json?v=${Date.now()}`;
...
function _handleDeepLinkJump() {
    const urlParams = new URLSearchParams(window.location.search);
    const paraId = urlParams.get('para');
    const searchTerm = urlParams.get('hl') || urlParams.get('highlight') || urlParams.get('phrase');
    const hash = window.location.hash ? window.location.hash.slice(1) : null;

    let targetEl = null;

    if (paraId) {
        targetEl = document.getElementById(paraId);
    }
    if (!targetEl && hash) {
        targetEl = document.getElementById(hash);
    }

    if (!targetEl && searchTerm) {
        const cleanTerm = searchTerm.toLowerCase().trim();
        const blocks = Array.from(document.querySelectorAll('#content-area p, #content-area li, #content-area blockquote, #content-area h1, #content-area h2, #content-area h3, #content-area td'));
        
        targetEl = blocks.find(b => (b.textContent || '').toLowerCase().includes(cleanTerm));
        if (!targetEl) {
            const terms = cleanTerm.split(/\s+/).filter(t => t.length > 2);
            if (terms.length > 0) {
                targetEl = blocks.find(b => {
    ...
    if (targetEl) {
        // Expand all parent <details> collapsibles so target element is visible
