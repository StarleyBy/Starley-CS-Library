...
function updatePreview() {
    ...
    if (bookPath && chapterId) {
        ...
        const relativeImagesDir = `${bookPath}/chapters/${parentFolder}/images/`;
        let imagesBase = (typeof window.getImageUrl === 'function')
            ? window.getImageUrl(relativeImagesDir)
            : `${typeof BASE_URL !== 'undefined' ? BASE_URL : './'}${relativeImagesDir}`;
    ...
    // --- Post-render styling & R2 image URL resolution ---
    const bookSelectEl = document.getElementById('select-book');
    const chapterSelectEl = document.getElementById('select-chapter');
    const currentBookPath = bookSelectEl ? bookSelectEl.value : '';
    let currentChapterId = chapterSelectEl ? chapterSelectEl.value : '';
    if (!currentChapterId && document.getElementById('export-filename')) {
        const fname = document.getElementById('export-filename').value;
        if (fname) currentChapterId = fname.replace(/(-ru|-starley|-he)?\.md$/, '');
    }

    let imgParentFolder = currentChapterId || 'chapter-01';
    const subMatchFolder = imgParentFolder.match(/^(chapter-\d+)-\d+$/);
    if (subMatchFolder) {
        imgParentFolder = subMatchFolder[1];
    }

    previewContainer.querySelectorAll('img').forEach(img => {
        img.classList.add('med-img');
        const rawSrc = img.getAttribute('src');
        if (rawSrc && currentBookPath) {
            ...
            if (!r2Prefix || !rawSrc.startsWith(r2Prefix)) {
                ...
                if (fileName && !fileName.includes('book-placeholder')) {
                    const relativePath = `${currentBookPath}/chapters/${imgParentFolder}/images/${fileName}`;
        ...
        previewBtn.addEventListener('click', () => {
            ...
            manifest.questions.forEach((q, i) => {
                ...
                const qImgs = (q.images && Array.isArray(q.images) && q.images.length > 0)
                    ? q.images
                    : (q.image ? String(q.image).split(',').map(s => s.trim()).filter(Boolean) : []);
                const qImgsHtml = qImgs.map(img => {
                    const src = img.startsWith('http') ? img : `${rootPath}${bookPath}/quiz/images/${img}`;
                    return `<img src="${src}" style="max-width:100%; max-height:200px; border-radius:4px; margin:10px 5px; cursor:zoom-in;" onclick="window.open('${src}', '_blank')">`;
                }).join('');

                const expImgs = (q.explanationImages && Array.isArray(q.explanationImages) && q.explanationImages.length > 0)
                    ? q.explanationImages
                    : (q.explanationImage ? [q.explanationImage] : []);
                const expImgsHtml = expImgs.map(img => {
                    const src = img.startsWith('http') ? img : `${rootPath}${bookPath}/quiz/images/${src}`;
                    return `<img src="${src}" style="max-width:100%; max-height:150px; border-radius:4px; margin-top:5px; margin-right:5px; cursor:zoom-in;" onclick="window.open('${src}', '_blank')">`;
                }).join('');
                ...
                html += `<div style="margin-bottom:20px; padding:10px; background:rgba(255,255,255,0.05); border-radius:8px;">
                    ...
                    ${qImgsHtml ? `<div style="display:flex; flex-wrap:wrap;">${qImgsHtml}</div>` : ''}
                    ...
                    <div style="border-top:1px solid #333; margin-top:10px; padding-top:10px;">
                        ...
                        ${expImgsHtml ? `<div style="display:flex; flex-wrap:wrap;">${expImgsHtml}</div>` : ''}
                    </div>
                </div>`;
            });
